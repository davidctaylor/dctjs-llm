...
request may contain multiple intents. Each intent can be classified into one of the following types:
- PAGE
- SECTION
- INVALID. 
...
These are the valid component types and their synonyms for that available for SECTIONS: 
- ACCORDION: expansion panel
- CARD: card
- CAROUSEL: slider
- LINK: button, CTA, call to action

If the request cannot be classified as either PAGE or SECTION classify it as INVALID. For example the request \
"create a section with a modal component" should be classified as INVALID.

Based on the input provided, return an array of intents for the user request. If the request contains \
the terms like 'Create a section', 'Add a Section' or 'New section', user intents should be grouped by \
SECTION. For example the following request:
